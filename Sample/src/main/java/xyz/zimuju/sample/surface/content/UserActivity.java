...
import xyz.zimuju.common.widget.BottomMenuView;
...
public class UserActivity extends BaseActivity implements OnClickListener, OnBottomDragListener, BottomMenuView.OnBottomMenuItemClickListener, OnHttpResponseListener {
    ...
    public void initEvent() {//必须调用
        ...
        userView.setOnDataChangedListener(new BaseView.OnDataChangedListener() {
    ...
    public void onBottomMenuItemClick(int intentCode) {
        ...
        switch (intentCode) {
            ...
            case MenuUtils.INTENT_CODE_QRCODE:
                ...
                break;
            default:
                String phone = StringUtils.getCorrectPhone(user.getPhone());
                if (StringUtils.isNotEmpty(phone, true) == false) {
                    return;
                }
                switch (intentCode) {
    ...
    public void onHttpResponse(int requestCode, String resultJson, Exception e) {
        ...
        try {//如果服务器返回的json一定在最外层有个data，可以用OnHttpResponseListenerImpl解析
            ...
            user = JSON.parseObject(String.valueOf(data), User.class);
        } catch (Exception e1) {
        ...
        switch (v.getId()) {
            case R.id.llUserTag:
                toActivity(EditTextInfoActivity.createIntent(context, "标签"
                        , StringUtils.getTrimedString(tvUserTag)), REQUEST_TO_EDIT_TEXT_INFO);
                break;
            default:
                break;
        }

...
public class MTextUtils {
    ...
    public MTextUtils(String StrText, int x, int y, int w, int h, int bgcolor, int textcolor, int alpha, int textsize) {
        mPaint = new Paint();
        mString = new Vector();
    ...
    public void initText() {
        ...
        // 对画笔属性的设置
        // mPaint.setARGB(this.mAlpha, Color.red(this.mFontColor), Color.green(this.mFontColor), Color.blue(this.mFontColor));
    ...
    public void getTextInfo() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (ch == '\n') {
                mRealLine++;// 真实的行数加一
                mString.addElement(this.mStrText.substring(startEnable, i));
                startEnable = i + 1;
                w = 0;
            } else {
                ...
                if (w > this.mTextWidth) {
                    ...
                    if (i == count - 1) {
                        mRealLine++;// 真实的行数加一
                        mString.addElement(this.mStrText.substring(startEnable,
                                count));
                    }
                }
            }

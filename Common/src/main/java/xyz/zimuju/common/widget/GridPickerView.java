...
public class GridPickerView extends BaseView<List<Entry<Integer, String>>> {
    ...
    public ArrayList<PickerConfig> getConfigList() {
        return configList;
    }
    ...
    public int getSelectedItemPosition(int tabPosition) {
        return configList.get(tabPosition).getSelectedItemPosition();
    }
     ...
     * @param lastList
     */
    public final void init(ArrayList<PickerConfig> configList, List<Entry<Integer, String>> lastList) {
        ...
        int tabWidth = configList.size() < 4 ? ScreenUtils.getScreenWidth(context) / configList.size() : 3;
        llGridPickerViewTabContainer.removeAllViews();
        for (int i = 0; i < configList.size(); i++) {//需要重新设置来保持每个tvTab宽度一致

            addTab(i, tabWidth, StringUtils.getTrimedString(configList.get(i)));
        }
        ...
        bindView(currentTabPosition, lastList, configList.get(currentTabPosition).getSelectedItemPosition());
    }
    ...
    private void addTab(final int tabPosition, int tabWidth, String name) {
        if (StringUtils.isNotEmpty(name, true) == false) {
            return;
        }
    ...
    public void bindView(final int tabPosition, List<Entry<Integer, String>> list, int itemPosition) {//GridView
        ...
        PickerConfig gpcb = configList.get(tabPosition);
        if (gpcb == null) {
            return;
        }
    ...
    public void doOnItemSelected(int tabPosition, int itemPosition, String itemName) {
        ...
        currentSelectedItemName = StringUtils.getTrimedString(itemName);

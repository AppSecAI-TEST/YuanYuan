...
public class FlashView extends FrameLayout {
    ...
    private void initUI(Context context) {
        ...
        LayoutInflater.from(context).inflate(R.layout.common_view_layout_slideshow, this, true);
        mLinearLayout = (LinearLayout) findViewById(R.id.linearlayout);
        mViewPager = (ViewPager) findViewById(R.id.bottom_pager);
    ...
    public void setImageUris(List<String> imageuris) {
        ...
        if (imageuris.size() <= 0) {// 如果得到的图片张数为0，则增加一张默认的图片
            imageUris.add("drawable://" + R.mipmap.common_ic_picture_loading);
        } else {
            if (imageuris.size() == 2) {
                isTwo = true;
...
//			ImageLoader.getInstance().displayImage(imageUris.get(i), imageView, options);
            GlideImageLoader.load(context, imageUris.get(i), imageView);
            imageViewsList.add(imageView);
            ImageView viewDot = new ImageView(getContext());
            if (i == 0) {
                viewDot.setBackgroundResource(R.mipmap.common_dot_white);
            } else {
                viewDot.setBackgroundResource(R.mipmap.common_dot_light);
            }
    ...
    private void setImageBackground(int selectItems) {
        for (int i = 0; i < dotViewsList.size(); i++) {
            if (i == selectItems % dotViewsList.size()) {
                dotViewsList.get(i).setBackgroundResource(R.mipmap.common_dot_white);
            } else {
                dotViewsList.get(i).setBackgroundResource(R.mipmap.common_dot_light);
            }
        }
    }

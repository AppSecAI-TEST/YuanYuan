...
public class ServerSettingActivity extends BaseActivity implements OnClickListener, OnBottomDragListener {
    ...
    public void initData() {//必须调用
        ...
        //获取并显网址
        etServerSettingNormal.setText(StringUtils.getNoBlankString(normalAddress));
        etServerSettingTest.setText(StringUtils.getNoBlankString(testAddress));
    ...
    private void saveAndExit(boolean isTest) {
        if (StringUtils.isNotEmpty(sharedPreferencesPath, true)
                && StringUtils.isNotEmpty(isTest ? testKey : normalKey, true)) {
            ...
            DataKeeper.save(context, sharedPreferencesPath, pathMode, isTest ? testKey : normalKey
                    , StringUtils.getNoBlankString(isTest ? etServerSettingTest : etServerSettingNormal));
    ...
    public void onDragBottom(boolean rightToLeft) {
        if (rightToLeft) {
            etServerSettingNormal.setText(StringUtils.getTrimedString(SettingUtil.URL_SERVER_ADDRESS_NORMAL_HTTP));
            etServerSettingTest.setText(StringUtils.getTrimedString(SettingUtil.URL_SERVER_ADDRESS_TEST));
            return;
        }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.tvServerSettingNormalSet) {
            ...
            toActivity(WebViewActivity.createIntent(context
                    , "正式服务器", StringUtils.getString(etServerSettingNormal)));
        } else if (v.getId() == R.id.tvServerSettingTestOpen) {
            toActivity(WebViewActivity.createIntent(context
                    , "测试服务器", StringUtils.getString(etServerSettingTest)));
        }
    }

 ...
 *
 * @author Lemon
 * @use <br> toActivity或startActivityForResult (PlacePickerWindow.createIntent(...), requestCode);
...
public class PlacePickerWindow extends BaseViewBottomWindow<List<Entry<Integer, String>>, GridPickerView> {
    ...
    public void initData() {//必须调用
        ...
        runThread(TAG + "initData", new Runnable() {
            ...
            public void run() {
                if (cityDB == null) {
                    cityDB = CityDB.getInstance(context, StringUtils.getTrimedString(
                            getIntent().getStringExtra(INTENT_PACKAGE_NAME)));
                }

                final ArrayList<PickerConfig> configList = new ArrayList<PickerConfig>();
                configList.add(new PickerConfig("", "浙江", 10));
                configList.add(new PickerConfig("", "杭州", 0));
    ...
    private synchronized List<Entry<Integer, String>> getList(int tabPosition, ArrayList<String> selectedItemList) {
        ...
        switch (level) {
            ...
            case PlaceUtil.LEVEL_CITY:
                nameList = cityDB.getProvinceAllCity(StringUtils.getTrimedString(selectedItemList.get(0)));
                break;

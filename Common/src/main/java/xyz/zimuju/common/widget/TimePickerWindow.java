 ...
 *
 * @author Lemon
 * @use <br> toActivity或startActivityForResult (TimePickerWindow.createIntent(...), requestCode);
...
public class TimePickerWindow extends BaseViewBottomWindow<List<Entry<Integer, String>>, GridPickerView> {
    ...
    public static Intent createIntent(Context context, int[] limitTimeDetail, int[] defaultTimeDetail) {
        ...
        if (limitTimeDetail != null && limitTimeDetail.length >= MIN_LENGHT) {
            minTimeDetail = TimeUtils.getTimeDetail(System.currentTimeMillis());//基本只会选后面的时间
    ...
    public void initData() {//必须调用
        ...
        if (defaultTimeDetails == null || defaultTimeDetails.length < MIN_LENGHT) {
            defaultTimeDetails = TimeUtils.getTimeDetail(System.currentTimeMillis());
        }
        ...
        runThread(TAG + "initData", new Runnable() {
            ...
            public void run() {
                ...
                configList.add(new PickerConfig(TimeUtils.NAME_HOUR, "" + selectedItemList.get(0)
                        , selectedItemList.get(0), 6, 4));
                configList.add(new PickerConfig(TimeUtils.NAME_MINUTE, "" + selectedItemList.get(1)
    ...
    private synchronized List<Entry<Integer, String>> getList(int tabPosition, ArrayList<Integer> selectedItemList) {
        int level = TimeUtils.LEVEL_HOUR + tabPosition;
        if (selectedItemList == null || selectedItemList.size() < MIN_LENGHT
                || TimeUtils.isContainLevel(level) == false) {
        ...
        boolean isContinuous = TimeUtils.fomerIsEqualOrBigger(maxTimeDetails, minTimeDetails);//范围是连续的

...
public class CacheManager {
    ...
    public <T> String getListPath(Class<T> clazz) {
        ...
        return StringUtils.isNotEmpty(classPath, true) ? classPath + KEY_LIST : null;
    }
    ...
    public <T> String getGroupPath(Class<T> clazz) {
        String classPath = getClassPath(clazz);
        return StringUtils.isNotEmpty(classPath, true) == false ? null : classPath + KEY_GROUP;
    }

    private SharedPreferences getSharedPreferences(String path) {
        return StringUtils.isNotEmpty(path, true) == false
                ? null : context.getSharedPreferences(StringUtils.getTrimedString(path), Context.MODE_PRIVATE);
    }
    ...
    public <T> List<T> getAllList(Class<T> clazz, String group) {
        return StringUtils.isNotEmpty(group, true) ? getList(clazz, group, -1, 0) : null;
    }
    ...
    public <T> List<T> getList(Class<T> clazz, String group, int start, int count) {
        ...
        if (StringUtils.isNotEmpty(group, true) == false) {
    ...
    public <T> List<String> getIdList(Class<T> clazz, String group) {
        ...
        return sp == null ? null : JSON.parseArray(sp.getString(StringUtils.getTrimedString(group), null), String.class);
    }
    ...
    public <T> void saveList(Class<T> clazz, String group, LinkedHashMap<String, T> map, int start, int pageSize) {
        ...
        if (StringUtils.isNotEmpty(group, true)) {
            group = StringUtils.getTrimedString(group);
    ...
    public <T> void save(Class<T> clazz, T data, String id, String group) {
        if (data == null || StringUtils.isNotEmpty(id, true) == false) {
